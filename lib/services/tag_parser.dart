...
class TagParser {
  ...
  static DateTime? _parseSemanticDateWithTime(String tagValue, DateTime now) {
    ...
    switch (datePartOnly) {
      ...
      // Dny v týdnu (příští výskyt daného dne)
      case 'po':
        baseDate = _getNextWeekday(now, DateTime.monday);
        break;
      case 'ut':
        baseDate = _getNextWeekday(now, DateTime.tuesday);
        break;
      case 'st':
        baseDate = _getNextWeekday(now, DateTime.wednesday);
        break;
      case 'ct':
        baseDate = _getNextWeekday(now, DateTime.thursday);
        break;
      case 'pa':
        baseDate = _getNextWeekday(now, DateTime.friday);
        break;
      case 'so':
        baseDate = _getNextWeekday(now, DateTime.saturday);
        break;
      case 'ne':
        baseDate = _getNextWeekday(now, DateTime.sunday);
        break;
  ...
  /// Najít příští výskyt daného dne v týdnu
  ///
  /// targetWeekday: 1=pondělí, 7=neděle (DateTime constants)
  ///
  /// Příklady:
  /// - Dnes je pondělí (1), hledám pondělí → vrátí pondělí za týden
  /// - Dnes je pondělí (1), hledám úterý (2) → vrátí zítra
  /// - Dnes je pátek (5), hledám pondělí (1) → vrátí příští pondělí (za 3 dny)
  static DateTime _getNextWeekday(DateTime from, int targetWeekday) {
    final fromDay = DateTime(from.year, from.month, from.day);
    final daysUntil = (targetWeekday - fromDay.weekday + 7) % 7;

    // Pokud je daysUntil == 0, znamená to že dnes JE ten den
    // → vrátíme příští týden (za 7 dní)
    return fromDay.add(Duration(days: daysUntil == 0 ? 7 : daysUntil));
  }
