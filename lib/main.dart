  ...
  runApp(
    MultiProvider(
      providers: [
        ...
        // ProfileBloc pro profil uživatele + rodina
        BlocProvider(
          create: (_) {
            final profileRepository = ProfileRepositoryImpl(db);
            return ProfileBloc(profileRepository)
              ..add(const LoadProfileEvent()); // Automaticky načíst profil
          },
        ),
        // PrankCubit pro AI pranky po dokončení úkolu
        BlocProvider(
          create: (_) => PrankCubit(PrankRepositoryImpl(db)),
        ),
        // ConnectivityCubit pro monitoring síťového připojení
        BlocProvider(
