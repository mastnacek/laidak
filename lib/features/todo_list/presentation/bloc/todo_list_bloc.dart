...
class TodoListBloc extends Bloc<TodoListEvent, TodoListState> {
  ...
  TodoListBloc(
    ...
    // Recurrence handlers (Todoist model)
    on<ContinueRecurrenceEvent>(_onContinueRecurrence);
    on<EndRecurrenceEvent>(_onEndRecurrence);
  }
  ...
  /// Handler: Pokračovat v opakování (posunout termín)
  ///
  /// User dokončil recurring úkol a potvrdil pokračování v dialogu.
  /// Posune due_date na další termín podle recurrence rule.
  Future<void> _onContinueRecurrence(
    ContinueRecurrenceEvent event,
    Emitter<TodoListState> emit,
  ) async {
    // ✅ Fail Fast: validace
    if (event.todoId <= 0) {
      emit(TodoListError('Neplatné ID úkolu: ${event.todoId}'));
      return;
    }

    try {
      // Posunout due_date na další termín
      await _repository.updateTodoDueDate(event.todoId, event.nextDate);

      // Reload todos
      add(const LoadTodosEvent());
    } catch (e) {
      emit(TodoListError('Chyba při posunu termínu: $e'));
    }
  }

  /// Handler: Ukončit opakování (smazat rule + complete todo)
  ///
  /// User dokončil recurring úkol a zvolil ukončení opakování v dialogu.
  /// Smaže RecurrenceRule a označí TODO jako completed.
  Future<void> _onEndRecurrence(
    EndRecurrenceEvent event,
    Emitter<TodoListState> emit,
  ) async {
    // ✅ Fail Fast: validace
    if (event.todoId <= 0) {
    ...
    try {
      // 1. Smazat RecurrenceRule
      await _repository.deleteRecurrenceRule(event.todoId);

      // 2. Označit TODO jako completed
      await _repository.toggleTodoStatus(event.todoId, true);

      // Reload todos
      add(const LoadTodosEvent());
    } catch (e) {
      emit(TodoListError('Chyba při ukončování opakování: $e'));
    }
  }
